CREATE TABLE [zzz].[ods__TM_Ticket_bkp_700Rollout]
(
...
[InsertDate] [datetime] NULL CONSTRAINT [DF__TM_Ticket__Inser__05A3D694] DEFAULT (getdate()),
[UpdateDate] [datetime] NULL CONSTRAINT [DF__TM_Ticket__Updat__0697FACD] DEFAULT (getdate()),
...
GO
ALTER TABLE [zzz].[ods__TM_Ticket_bkp_700Rollout] ADD CONSTRAINT [PK_TM_Ticket] PRIMARY KEY CLUSTERED  ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_BusinessKey] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([acct_id], [plan_event_id], [ticket_status], [section_id], [row_id], [seat_num]) INCLUDE ([num_seats], [ssbPriceCode])
GO
CREATE NONCLUSTERED INDEX [IDX_TM_Tkt_ticket_status] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([num_seats], [ticket_status], [acct_id], [plan_event_id], [seat_num], [section_id], [row_id]) INCLUDE ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([ticket_status])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status_add_datetime] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([ticket_status], [add_datetime]) INCLUDE ([block_purchase_price], [event_id], [event_name], [num_seats], [plan_event_id], [price_code], [Price_code_desc], [row_id], [seat_num], [section_id])
GO
CREATE NONCLUSTERED INDEX [Idx_upd_datetime_event_id] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([upd_datetime], [acct_id], [event_id])
GO
CREATE NONCLUSTERED INDEX [IDX_UpdateDate] ON [zzz].[ods__TM_Ticket_bkp_700Rollout] ([UpdateDate])
GO
