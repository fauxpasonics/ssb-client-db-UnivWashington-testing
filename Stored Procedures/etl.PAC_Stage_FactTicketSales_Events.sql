...
/***** Revision History
 ...
 Jeff Barberio on 2018-07-26: Adjusted RevenueTicket, RevenueTotal, FullPrice logic
, Reviewed By: Zach Frick on 2018-07-26

*****/


CREATE PROCEDURE [etl].[PAC_Stage_FactTicketSales_Events] 
( 
...
SELECT 
	...
	, CASE WHEN ISNULL(tkt.MINPAYMENTDATE, tkt.ORDERDATE) IS NULL THEN 0 ELSE CONVERT(NVARCHAR(8), ISNULL(tkt.MINPAYMENTDATE, tkt.ORDERDATE), 112) END DimDateId 
	, CASE WHEN ISNULL(tkt.MINPAYMENTDATE, tkt.ORDERDATE) IS NULL THEN 0 ELSE datediff(second, cast(ISNULL(tkt.MINPAYMENTDATE, tkt.ORDERDATE) as date), ISNULL(tkt.MINPAYMENTDATE, tkt.ORDERDATE)) END DimTimeId 
	...
	, ( ISNULL(tkt.ORDTOTAL,0) - ISNULL(tkt.OrderConvenienceFee,0) - ISNULL(tkt.ORDFEE,0) )  RevenueTicket --jbarberio Adjusted logic to subtract surcharge/fees (These are now built into ORDTOTAL)
	, ISNULL(tkt.OrderConvenienceFee,0) RevenueFees
	, ISNULL(tkt.ORDFEE,0) RevenueSurcharge
	, CAST(0 AS DECIMAL(18,6)) RevenueTax	
	, ISNULL(tkt.ORDTOTAL,0) RevenueTotal --jbarberio removed the addition of surcharge/fees as these are built into ORDTOTAL now

	, ( ISNULL(tkt.ORDTOTAL,0) + ( ISNULL(tkt.ORDQTY,0) * ISNULL(tkt.E_DAMT,0) )) FullPrice --jbarberio removed the addition of surcharge/fees as these are built into ORDTOTAL now
