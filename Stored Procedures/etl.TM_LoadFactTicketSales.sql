...
/*
DROP TABLE #stgFactTicketSales
DROP TABLE #WorkingSet

EXEC [dbo].[SSBHashFieldSyntaxZF] 'dbo.FactTicketSales', 'FactTicketSalesId, DeltaHashKey, SourceSystem, CreatedBy, UpdatedBy, CreatedDate, UpdatedDate, IsDeleted, DeleteDate,SSID_event_id, SSID_section_id, SSID_row_id, SSID_seat_num', ''
*/
...
--SELECT q.*
--INTO #ReprocessQueue
--FROM etl.STG_FactTicketSales_ReporcessTransactionQueue q
--LEFT OUTER JOIN #WorkingSet tkt ON q.event_id = tkt.event_id AND q.section_id = tkt.section_id AND q.row_id = tkt.row_id AND q.seat_num = tkt.seat_num
...
--SELECT tkt.*
--FROM ods.TM_vw_Ticket tkt
--INNER JOIN #ReprocessQueue q ON q.event_id = tkt.event_id AND q.section_id = tkt.section_id AND q.row_id = tkt.row_id AND q.seat_num = tkt.seat_num

--EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Fact Load', @LogEventDefault, 'Reprocess Trans Added to Working Set', @ExecutionId
...
SELECT 
	...
	--SELECT COUNT(*)
	FROM #WorkingSet tkt
	LEFT OUTER JOIN dbo.DimItem (nolock) dItem ON CASE WHEN ISNULL(tkt.plan_event_id,0) = 0 THEN tkt.event_id ELSE tkt.plan_event_id END = dItem.SSID_event_id AND dItem.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dItem.DimItemId > 0
	LEFT OUTER JOIN dbo.DimSeason (nolock) dSeason ON dItem.DimSeasonId = dSeason.DimSeasonId AND dSeason.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dSeason.DimSeasonId > 0
	LEFT OUTER JOIN dbo.DimEvent (nolock) dEvent ON tkt.event_id = dEvent.SSID_event_id AND dEvent.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dEvent.DimEventId > 0
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomer ON tkt.acct_id = dCustomer.AccountId AND dCustomer.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomer.CustomerType = 'Primary' AND dCustomer.DimCustomerId > 0
		
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerSalesRep ON tkt.acct_rep_id = dCustomerSalesRep.AccountId AND dCustomerSalesRep.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomerSalesRep.CustomerType = 'Primary' AND dCustomerSalesRep.DimCustomerId > 0
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerTransSalesRep ON tkt.orig_acct_rep_id = dCustomerTransSalesRep.AccountId AND dCustomerTransSalesRep.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dCustomerTransSalesRep.CustomerType = 'Primary' AND dCustomerTransSalesRep.DimCustomerId > 0
	
	LEFT OUTER JOIN dbo.DimPlan (nolock) dPlan ON tkt.plan_event_id = dPlan.SSID_event_id AND dPlan.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dPlan.DimPlanId > 0
	LEFT OUTER JOIN dbo.DimSeat (nolock) dSeat ON dSeason.ManifestId = dSeat.SSID_manifest_id AND tkt.section_id = dSeat.SSID_section_id AND tkt.row_id = dSeat.SSID_row_id AND tkt.seat_num = dSeat.SSID_seat AND dSeat.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dseat.DimSeatId > 0	
	LEFT OUTER JOIN dbo.DimSalesCode (nolock) dSalesCode ON CASE WHEN TRY_CAST(tkt.sell_location AS INT) IS NULL THEN dSalesCode.SalesCode ELSE CONVERT(NVARCHAR,dSalesCode.SSID_sell_location_id) END = CONVERT(NVARCHAR,tkt.sell_location) AND dSalesCode.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dSalesCode.DimSalesCodeId > 0
	LEFT OUTER JOIN dbo.DimPromo (nolock) dPromo ON tkt.promo_code = dPromo.ETL_SSID_promo_code AND dPromo.DimPromoID > 0
	LEFT OUTER JOIN dbo.DimClassTM (nolock) dctm ON dctm.ClassName = tkt.class_name AND dctm.ETL_SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dPlan.DimPlanId > 0 AND dctm.DimClassTMId > 0 

	LEFT OUTER JOIN dbo.DimPriceCodeMaster (nolock) dpcm ON tkt.ssbPriceCode = dpcm.PriceCode AND dpcm.DimPriceCodeMasterId > 0
	LEFT OUTER JOIN dbo.DimPriceCode (nolock) dPriceCode ON tkt.event_id = dPriceCode.SSID_event_id AND tkt.ssbPriceCode = dPriceCode.SSID_price_code AND dPriceCode.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dPriceCode.DimPriceCodeId > 0
	LEFT OUTER JOIN dbo.DimPriceCode (nolock) dPriceCodePlan ON tkt.plan_event_id = dPriceCodePlan.SSID_event_id AND tkt.ssbPriceCode = dPriceCodePlan.SSID_price_code AND dPriceCodePlan.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem')) AND dPriceCodePlan.DimPriceCodeId > 0

	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedger ON dLedger.ETL_SSID_ledger_id = tkt.ledger_id AND dLedger.DimLedgerId > 0 AND dLedger.ETL_SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem'))
	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedgerEventPC ON dLedgerEventPC.LedgerCode = dPriceCode.LedgerCode AND dLedgerEventPC.DimLedgerId > 0 AND dLedgerEventPC.ETL_SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem'))
	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedgerPlanPC ON dLedgerPlanPC.LedgerCode = dPriceCodePlan.LedgerCode AND dLedgerPlanPC.DimLedgerId > 0 AND dLedgerPlanPC.ETL_SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem'))		
	...
	--ALTER INDEX [IDX_LoadKey] ON dbo.FactTicketSales REBUILD
	--ALTER INDEX [IDX_DeltaHashKey] ON dbo.FactTicketSales REBUILD
		
	EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Fact Load', @LogEventDefault, 'FactTicketSales Indexes Rebuilt', @ExecutionId
...
USING (select * from #stgFactTicketSales) as mySource
     ...
     ON
		myTarget.SourceSystem = (SELECT etl.fnGetClientSetting('TM-SourceStyem'))
...
WHEN NOT MATCHED THEN INSERT
    ...
    VALUES (
		...
		, (SELECT etl.fnGetClientSetting('TM-SourceStyem')) --SourceSystem
