...
CREATE PROCEDURE [etl].[Load_FactTicketSales_PAC_Events_V2] 
( 
...
INSERT INTO etl.vw_FactTicketSales
( 
	ETL__SourceSystem
	--, ETL__CreatedBy AMEITIN 2017/10/02
...
SELECT 
	...
	--, SUSER_NAME() ETL__CreatedBy AMEITIN 2017/10/02
	, GETDATE() ETL__CreatedDate, GETDATE() ETL__UpdatedDate
	...
	, CAST(CASE WHEN dplan.DimPlanId > 0 AND dEvent.DimEventId = ISNULL(dPlan.Config_PlanRenewableDimEventId, dPlan.PlanRenewableDimEventId) 
        THEN ISNULL(tkt.ORDQTY,0) ELSE 0 END AS INT) QtySeatRenewable --AMEITIN 2017/10/02
