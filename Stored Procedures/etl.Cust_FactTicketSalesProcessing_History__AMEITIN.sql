...
CREATE PROCEDURE [etl].[Cust_FactTicketSalesProcessing_History__AMEITIN]
(
	@BatchId INT = 0,
...
FROM    [dbo].[FactTicketSales_V2_History]  fts 
		...
		INNER JOIN [ro].[vw_DimPriceCode] pc ON fts.DimPriceCodeId = pc.DimPriceCodeId
...
--SELECT DISTINCT Seasonname, ItemCode, PriceCode, PriceCodeDesc
FROM    [dbo].[FactTicketSales_V2_History] fts 
        INNER JOIN [ro].[vw_DimItem] di ON fts.DimItemId = di.DimItemId
		INNER JOIN [ro].[vw_DimSeason] ds ON fts.DimSeasonId = ds.DimSeasonId
		INNER JOIN [ro].[vw_DimPriceCode] pc ON fts.DimPriceCodeId = pc.DimPriceCodeId
WHERE di.ItemCode LIKE '%[0-9]'
--AND di.ItemClass = 'Event'
AND ds.seasonname IN ('2015 Husky Football', '2016 Husky Football')
...
FROM    [dbo].[FactTicketSales_V2_History] fts 
		INNER JOIN [ro].[vw_DimEvent] de ON fts.DimEventId = de.DimEventId
		INNER JOIN [ro].[vw_DimPriceCode] pc ON fts.DimPriceCodeId = pc.DimPriceCodeId
WHERE 1=1
...
UPDATE f
SET f.IsComp = 1
FROM [dbo].[FactTicketSales_V2_History]  f
	JOIN [ro].[vw_DimPriceCode] dpc
...
FROM    [dbo].[FactTicketSales_V2_History]  fts
        INNER JOIN [ro].[vw_DimPriceCode] pc ON pc.DimPriceCodeId = fts.DimPriceCodeId
WHERE 1=1
...
UPDATE dbo.FactTicketSales_V2_History
SET DimTicketTypeId = '22'
WHERE DimpriceCodeId IN (
'17540'
,'9309'
,'5530'
,'3725'
...
FROM dbo.FactTicketSales_V2_History fts
INNER JOIN dbo.DimPriceCode_V2 dpc on fts.DimpriceCodeId = dpc.DimpriceCodeid  
WHERE PriceCode IN ('S','SC')
...
UPDATE fts
SET DimTicketTypeId = '20'
FROM dbo.FactTicketSales_V2_History fts
INNER JOIN dbo.DimEvent_V2 dpc on fts.DimEventId = dpc.DimEventid  
WHERE EventName like '%BFE%'

UPDATE dPlan
SET Config_PlanRenewableDimEventId = '25'
, ETL__UpdatedDate = GETUTCDATE()
FROM dbo.DimPlan_V2 dPlan
WHERE dPlan.DimPlanId IN (
'2'
,'20'
,'22'
,'23'
...
UPDATE dPlan
SET Config_PlanRenewableDimEventId = '225'
, ETL__UpdatedDate = GETUTCDATE()
FROM dbo.DimPlan_V2 dPlan
WHERE dPlan.DimPlanId IN (
'33'
