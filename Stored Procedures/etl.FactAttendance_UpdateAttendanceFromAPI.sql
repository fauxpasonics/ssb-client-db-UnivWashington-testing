...
BEGIN
	...
	DECLARE @TMSourceSystem NVARCHAR(25) = (SELECT etl.fnGetClientSetting('TM-SourceStyem'))
	...
	FROM #seat a
	INNER JOIN dbo.DimCustomer (NOLOCK) dc ON a.acct_id = dc.AccountId AND dc.CustomerType = 'Primary' AND dc.SourceSystem = @TMSourceSystem
	...
	, @TMSourceSystem ETL_SourceSystem
	...
	LEFT OUTER JOIN dbo.DimSeat (NOLOCK) dst 
		...
		AND dst.SourceSystem = @TMSourceSystem
