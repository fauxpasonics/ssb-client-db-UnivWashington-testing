...
FROM (
	...
	, (CAST(tex.te_purchase_price as DECIMAL(18,6)) / tex.num_seats) ResoldPurchasePrice
	, (CAST(tex.te_buyer_fees_hidden AS DECIMAL(18,6)) / tex.num_seats) ResoldFees
	, ROW_NUMBER() OVER(PARTITION BY tex.event_id, tex.section_id, tex.row_id, sl.Seat ORDER BY tex.add_datetime desc) AS RowRank	
	FROM ods.TM_Tex tex
	INNER JOIN dbo.Lkp_SeatList sl ON sl.Seat >= tex.seat_num  AND sl.Seat < (tex.seat_num + tex.num_seats)
