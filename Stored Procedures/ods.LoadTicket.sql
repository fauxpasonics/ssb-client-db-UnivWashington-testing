...
CREATE PROCEDURE [ods].[LoadTicket]
(
	@BatchId BIGINT = 0,
...
--SET NOCOUNT ON;
	...
	DECLARE @SourceFileName nvarchar(255) = ( SELECT TOP 1 CASE WHEN ISNULL(SourceFileName,'') <> '' THEN SourceFileName ELSE CONVERT(NVARCHAR(255), NEWID()) END FROM src.TM_Ticket )


	insert INTO archive.TM_Ticket
	(
		CreatedDate, 
		event_name, section_name, row_name, num_seats, ticket_status, acct_id, upd_datetime, seq_num, block_purchase_price, order_num, order_line_item,
		...
		other_info_8, other_info_9, other_info_10, acct_Rep_id, acct_rep_full_name, tran_type, section_id, row_id, promo_code, retail_ticket_type, retail_qualifiers,
		paid_amount, owed_amount, add_datetime, add_user, renewal_ind, SourceFileName, return_reason, return_reason_desc, expanded, delivery_method,
		delivery_method_name, delivery_instructions, delivery_name_first, delivery_name_last, delivery_addr1, delivery_addr2, delivery_addr3, delivery_city,
		delivery_state, delivery_zip, delivery_zip_formatted, delivery_country, delivery_phone, delivery_phone_formatted, delivered_date, group_sales_name,
		ledger_id, pc_ticket, pc_tax, pc_licfee, pc_other1, pc_other2, tax_rate_a, tax_rate_b, tax_rate_c, orig_acct_rep_id, ticket_seq_id
	)
	...
	SELECT getdate() CreatedDate,
		...
		plan_event_id, plan_event_name, seat_num, last_Seat, other_info_1, other_info_2, other_info_3, other_info_4, other_info_5, other_info_6, other_info_7,
		other_info_8, other_info_9, other_info_10, acct_Rep_id, acct_rep_full_name, tran_type, section_id, row_id, promo_code, retail_ticket_type, retail_qualifiers,
		paid_amount, owed_amount, add_datetime, add_user, renewal_ind, @SourceFileName SourceFileName, return_reason, return_reason_desc, expanded, delivery_method,
		delivery_method_name, delivery_instructions, delivery_name_first, delivery_name_last, delivery_addr1, delivery_addr2, delivery_addr3, delivery_city,
		delivery_state, delivery_zip, delivery_zip_formatted, delivery_country, delivery_phone, delivery_phone_formatted, delivered_date, group_sales_name,
		ledger_id, pc_ticket, pc_tax, pc_licfee, pc_other1, pc_other2, tax_rate_a, tax_rate_b, tax_rate_c, orig_acct_rep_id, ticket_seq_id
	FROM src.TM_Ticket
	...
	*/
	...
	DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.TM_Ticket),'0');	
...
MERGE ods.TM_Ticket as myTarget

USING (SELECT * FROM src.vw_TM_Ticket) AS mySource ON 
...
WHEN MATCHED AND mySource.HashKey <> myTarget.HashKey
THEN UPDATE SET
...
USING (SELECT * FROM src.vw_TM_Ticket_Returns) AS mySource ON 
