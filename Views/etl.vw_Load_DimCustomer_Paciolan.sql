...
--select count(*) from [etl].[vw_TI_Load_DimCustomer]
...
CREATE VIEW [etl].[vw_Load_DimCustomer_Paciolan] AS 
(
...
*/
		...
		SELECT 
			...
			/*AddressPrimary*/
			, CASE WHEN PrimaryAddressType = 'B' THEN LEFT(PrimaryAddressStreet,500)
			...
			, CASE WHEN PrimaryAddressType = 'B' THEN LEFT(PrimaryAddressCity,200)
				ELSE COALESCE(Address2City, Address3City, Address4City, Address5City) END AS [AddressPrimaryCity] 
			, CASE WHEN primaryaddresstype = 'B' THEN LEFT(PrimaryAddressState,200)
				ELSE COALESCE(Address2state, Address3State, Address4State, Address5State) END AS [AddressPrimaryState] 
			, CASE WHEN PrimaryAddressType = 'B' THEN LEFT(PrimaryAddressZipCode,25)
			...
			, CASE WHEN PrimaryAddressType = 'B' THEN LEFT(PrimaryAddressCountry,200)
			...
			, CAST(HomePhone AS NVARCHAR(25)) PhonePrimary
			...
			/*Email*/
			, COALESCE(p.[PersonalEmail],P.BusinessEmail, p.OtherEmail) AS [EmailPrimary]
			...
			, p.External_number AS [ExtAttribute31]
			...
			, CAST(COALESCE(PacCreateDate,'1900-01-01') AS DATETIME) [CreatedDate]
			, UpdatedDate [SSUpdatedDate]
			, 0 AS ISDeleted
			, NULL [AccountId]
			, CAST(CASE WHEN FullName LIKE '%,%' THEN 0 ELSE 1 END AS BIT) IsBusiness
			, 'Paciolan-' + [p].[Patron]  customer_MatchKey

--		select TOP 10 *
		FROM etl.vwTI_PatronMDM p
		WHERE 1=1
		AND (PacCreateDate > (GETDATE() - 10) OR UpdatedDate > (GETDATE() - 10)) 	
